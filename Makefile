...
DIST_COMMON = README $(am__configure_deps) $(srcdir)/Makefile.am \
	...
	$(top_srcdir)/configure ABOUT-NLS COPYING INSTALL NEWS THANKS \
	TODO
...
SUBDIRS = doc nid3lib src po config test 
DISTCLEANFILES = $(srcdir)/src/libintl.h
...
distdir: $(DISTFILES)
	$(am__remove_distdir)
	mkdir $(distdir)
	$(mkdir_p) $(distdir)/config $(distdir)/m4 $(distdir)/po $(distdir)/src
...
#	-cp -p $(srcdir)/debian/* $(distdir)/debian
	-mkdir $(distdir)/packaging
	for f in prototype request copyright; do \
		cp -p $(srcdir)/packaging/$$f $(distdir)/packaging; \
	done
	for f in $(srcdir)/packaging/*.in; do \
		BASE=`basename $$f .in`; \
		sed s/\@VERSION\@/$(VERSION)/ $$f \
			> $(distdir)/packaging/$$BASE; \
	done
