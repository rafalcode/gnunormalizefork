...
int main(int argc, char *argv[])
{
    ...
    while ((c = getopt_long(argc, argv, "hVnvqbmcT:l:g:a:t:w:", longopts, NULL)) != EOF) {
        switch(c) {
            case 'a':
                ...
                /* check if characters "dB" or "dBFS" follow the digits */
                while(isspace(*p))
                    p++;
                if (strncaseeq(p, "db", 2)) { /* amplitude given as dBFS */
                    if (target > 0) {
                        target = -target;
                        fprintf(stderr, _("%s: normalizing to %f dBFS\n"), progname, target);
                    }
                    target = DBFSTOAMP(target); /* translate to fraction */

                } else {
                    if (target < 0 || target > 1.0) { /* amplitude given as fraction */
    ...
    if (mix_mode && batch_mode) {
        fprintf(stderr, _("%s: error: the -m and -b options are mutually exclusive\n"), progname);
        exit(1);
    }
    ...
    /* get sizes of all files by running stat on them, for progress calculation */
    nfiles = 0;
    ...
    if (do_compute_levels) { /* OK this is more nitty gritty now ... we're going in and chekcing the level */
        compute_levels(sis, fnames, nfiles); /* main call to compute-levels */
    ...
    if (batch_mode) /* if gain_in_decibels, then dBdiff is the original specified gain */
        if (!gain_in_decibels)
            dBdiff = FRACTODB(gain);

    if (do_apply_gain) { /* Apply the gain */

        if (batch_mode && verbose >= VERBOSE_PROGRESS) { /* print adjust message for batch mode */
        ...
        for (i = 0; i < nfiles; i++) {
            ...
            ret = apply_gain(fnames[i], gain, psi); /* big call to apply gain over in adjust.c */

            if (ret == -1) {
                fprintf(stderr, _("%s: error applying adjustment to %s: %s\n"), progname, fnames[i], strerror(errno));
            } else {
                if (ret == 0) { /* gain was not applied */
                    if (!batch_mode) {
                        if (verbose >= VERBOSE_PROGRESS)
                            fprintf(stderr, _("%s already normalized, not adjusting..."), fnames[i]);
                    }
                } else { /* gain WAS applied */
 ...
 */
double average_levels(struct signal_info *sis, int nlevels, double threshold)
{
    int i, files_to_avg;
