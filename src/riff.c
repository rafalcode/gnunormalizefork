...
riff_t riff_open(const char *fname, int mode) /* a convenience for riff_fdopen: grabs a file descriptor for it */
{
    ...
    switch (mode) {
        ...
        case RIFF_WRONLY:
            open_mode = O_WRONLY | O_CREAT | O_TRUNC; break;
        case RIFF_RDWR:
            open_mode = O_RDWR | O_CREAT; break;
        default:
            errno = EACCES;
            return NULL;
    }
...
static int _read_chunk_start(riff_t riff, riff_chunk_t *chnk)
{
...
int riff_descend(riff_t riff, riff_chunk_t *chnk, riff_chunk_t *par_chnk, int search)
{
    ...
    if (par_chnk) {
        ...
        if (offset < par_chnk->offset + 4 || offset >= par_chnk->offset + par_chnk->size) {
            return 0;
        }
    }
...
int riff_create_chunk(riff_t riff, riff_chunk_t *chnk) /* not really about creating a chunk from zero; return values: -1 means error; 0 means success */
{
    uint32_t size;
    ...
    if (fwrite(&chnk->id, 4, 1, riff->fp) < 1 || fwrite(&size, 4, 1, riff->fp) < 1)
        return -1;
...
int riff_ascend(riff_t riff, riff_chunk_t *chnk) /* return values: -1 means error; 0 means success */
{
    long offset;
    uint32_t size;
...
FILE *riff_stream(riff_t riff) /* simply return the FILE ptr which is inside the riff_t struct */
{
    return riff->fp;
}

/* seek to beginning of chunk, then return stream */
FILE *riff_chunk_stream(riff_t riff, riff_chunk_t *chnk) /* simply does a seek_set on riff->fp */
{
...
static int _riff_walk(riff_t riff, riff_chunk_t *par_chnk, int depth)
{
    riff_chunk_t chnk;
    char id_s[5], typ_s[5];
    int i, ret;
    ...
    while (1) { /* infinite loop */
        ...
        if (chnk.id == RIFFID_LIST || chnk.id == RIFFID_RIFF) {
            ...
            _riff_walk(riff, &chnk, depth + 1); /* recursive */
            for (i = 0; i < depth; i++)
                printf("     ");
            printf("    )\n");
        } else {
...
int main(int argc, char *argv[])
{
    ...
    chnk.id = riff_string_to_fourcc("fmt "); /* the famed string right there */
