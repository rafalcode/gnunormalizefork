...
static inline long get_sample(unsigned char *pdata, int bytes_per_sample)
{
    long sample;
...
static inline void put_sample(long sample, unsigned char *pdata, int bytes_per_sample)
{
    switch(bytes_per_sample) {
        case 1:
 ...
 * available.
 */
static int _do_apply_gain(int read_fd, int write_fd, char *filename, double gain, struct signal_info *si) /* does the real work */
{
    AFfilehandle fhout;
    ...
    /* The two things we start with and which start unrelated are the AFfilesetup "setup" the AFfilehandle "fhin" */
    ...
    AFfilehandle fhin = afOpenFD(read_fd, "r", NULL);
    ...
    int channels = afGetChannels(fhin, AF_DEFAULT_TRACK);
    afInitChannels(setup, AF_DEFAULT_TRACK, channels);

    int samp_fmt, src_samp_width;
    afGetSampleFormat(fhin, AF_DEFAULT_TRACK, &samp_fmt, &src_samp_width);
    int dst_samp_width = src_samp_width; /* the dest sampformat will be the same as the src: a common operational pattern */
    if (output_bitwidth) {
    ...
    while ((frames_recvd = afReadFrames(fhin, AF_DEFAULT_TRACK, src_buf, frames_in_buf)) > 0) {
        ...
        if (lut) {
            ...
            if (use_limiter_this_file && verbose >= VERBOSE_INFO)
                fprintf(stderr, _("%s: Warning: no lookup table available; this may be slow...\n"), progname);
            ...
            for (c = 0; c < channels; c++) {
                ...
                for (i = 0; i < frames_recvd; i++) {
                    ...
                    sample_d = sample * gain; /* apply the gain to the sample */
    ...
    } /* end of big while on the frames received */
 ...
 */
int apply_gain(char *filename, double gain, struct signal_info *si) /* calls _do_apply_gain() */
{
    int i;
    ...
    if (i >= 4) {
        suffix = filename + i - 4;
        if (strncaseeq(suffix, ".mp3", 4) || strncaseeq(suffix, ".mp2", 4))
    ...
    if (xrename(tmpfile, filename) == -1) {
        fprintf(stderr, _("%s: error moving %s to %s: %s\n"), progname, tmpfile, filename, strerror(errno));
        free(tmpfile);
        return -1;
    }
    free(tmpfile);
