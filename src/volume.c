...
typedef struct /* mysterious datasmooth_t */
{
    double *buf;
    int buflen;  /* elements allocated to buffer: same as n? */
...
double signal_max_power(char *filename, struct signal_info *si)
{
    ...
    AFfilehandle fhin = afOpenFile(filename, "r", NULL);
    if (fhin == AF_NULL_FILEHANDLE)
        goto error1;

    /* pass back format info in *si I think he means to *si */
    ...
    afSetVirtualSampleFormat(fhin, AF_DEFAULT_TRACK, AF_SAMPFMT_TWOSCOMP, samp_width); /* weird, this always returns 0 */

    bytes_per_sample = (si->bits_per_sample - 1) / 8 + 1;
    samplemax = (1 << (bytes_per_sample * 8 - 1)) - 1;
    samplemin = -samplemax - 1;
    AFframecount framecount = afGetFrameCount(fhin, AF_DEFAULT_TRACK);
    ...
    } while (!last_window); /* end of big do */
