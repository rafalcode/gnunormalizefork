...
AFfilehandle afOpenFile(const char *filename, const char *mode, AFfilesetup setup) /* prepares flags, gets the fd for the setup parameter and then calls afOpenFD */
{
  int fd, flags;
  if (mode[0] == 'w')
...
AFfilehandle afOpenFD(int fd, const char *mode, AFfilesetup setup)
{
  ...
  AFFilehandle newfh = (AFfilehandle)malloc(sizeof(struct _AFfilehandle));
  if (newfh == NULL) {
  ...
  if (mode[0] == 'w') {
	...
	|| (setup->sample_format != AF_SAMPFMT_TWOSCOMP && setup->sample_format != AF_SAMPFMT_UNSIGNED)
  ...
  } else { /* finished with 'write', now for reading: */
...
int afCloseFile(AFfilehandle fh)
{
  if (fh) {
...
AFfilesetup afNewFileSetup(void) /* Important one: creates a filesetup struct with some basic defaults. */
{
  AFfilesetup retval;
  retval = (AFfilesetup)malloc(sizeof(struct _AFfilesetup));
  if (retval) { /* set defaults: WAV, 44.1k, mono, 16-bit LE */
...
static inline int _afGetFrameSize(AFfilehandle fh, int track, int expand3to4)
{
...
int afReadFrames(AFfilehandle fh, int track, void *buffer, int frame_count)
{
 ...
 * care.
 */
int afWriteFrames(AFfilehandle fh, int track, void *buffer, int frame_count)
{
...
float afGetFrameSize(AFfilehandle fh, int track, int expand3to4)
{
  return (float)(_afGetFrameSize(fh, track, expand3to4));
}

AFfileoffset afGetTrackBytes(AFfilehandle fh, int track)
{
  return fh->data_chnk.size;
}

AFframecount afGetFrameCount(AFfilehandle fh, int track)
{
  int framesize;
  uint32_t tracklen;
...
void afInitByteOrder(AFfilesetup setup, int track, int byte_order)
{
...
void afInitSampleFormat(AFfilesetup setup, int track, int sample_format, int sample_width)
{
  setup->sample_format = sample_format;
  setup->sample_width = sample_width;
}

int afGetFileFormat(AFfilehandle fh, int *version)
{
  if (version)
    *version = 0;
  return AF_FILE_WAVE;
...
int afGetByteOrder(AFfilehandle fh, int track)
{
  return AF_BYTEORDER_LITTLEENDIAN;
}

int afGetChannels(AFfilehandle fh, int track)
{
  return fh->fmt.channels;
}

void afGetSampleFormat(AFfilehandle fh, int track, int *sampfmt, int *sampwidth)
{
  if (sampfmt) {
    if (fh->fmt.bits_per_sample <= 8)
...
double afGetRate(AFfilehandle fh, int track)
{
 ...
 * noop.  */
int afSetVirtualSampleFormat(AFfilehandle fh, int track, int sampfmt, int sampwidth) /* simply always returns 0!! */
{
  return 0;
}

int afSyncFile(AFfilehandle fh)
{
